...
int main() {
	...
	mc_Hero mc_HeroList[] = {
		...
		mc_CreateHero("Hunter",        "hunter",    2200, 35, 6.5f, 1.4f),
		
		mc_CreateHero("Hypnotist",     "hypnotist", 2310, 28, 3.0f, 1.2f),
	};
	
	mc_Troop mc_TroopList[] = {
		// Do enumerado PIRATE_EPOCH
		
		mc_CreateTroop(655,  55,   ONLY_GROUND,           1.0f,  1.2f, 2.0f),
	...
	mc_Card mc_CardList[] = {
	    ...
	    mc_CreateCardTroop("Crusher",          "crusher",     1, FEUDAL_JAPAN,         7, mc_TroopList[8],  1), // 12
	    mc_CreateCardTroop("Samurai Hunter",   "shunter",     1, FEUDAL_JAPAN,         4, mc_TroopList[9],  1), // 13
	    
	    mc_CreateCardTroop("Plague Assasin",   "plaguek",     1, DARK_PLAGUE_ERA,      3, mc_TroopList[10], 1), // 14
	    mc_CreateCardTroop("Ghost Musketeer",  "ghost",       1, DARK_PLAGUE_ERA,      3, mc_TroopList[11], 1), // 15
	    mc_CreateCardTroop("Mosquitoes",       "bugs",        1, DARK_PLAGUE_ERA,      2, mc_TroopList[12], 5), // 16
	    mc_CreateCardSpell("Scary Fog",        "fog",         1, DARK_PLAGUE_ERA,      4, mc_SpellList[2]    ), // 17
	    mc_CreateCardTroop("Reaper",           "reaper",      1, DARK_PLAGUE_ERA,      6, mc_TroopList[13], 1), // 18
	    mc_CreateCardSpell("Sledgehammer",     "sledge",      1, DARK_PLAGUE_ERA,      5, mc_SpellList[3]    ), // 19
	    
	    mc_CreateCardTroop("Gun Dealer",       "gundealer",   1, TROUBLES_IN_FRANCE,   4, mc_TroopList[14], 1), // 20
	    mc_CreateCardBuild("Cannon",           "cannon",      1, TROUBLES_IN_FRANCE,   3, mc_BuildList[1]    ), // 21
	    
	    mc_CreateCardTroop("Nun",              "nun",         1, POST_TYRANNY_WORLD,   4, mc_TroopList[15], 1), // 22
	    mc_CreateCardTroop("H-Robot",          "hrobot",      1, POST_TYRANNY_WORLD,   8, mc_TroopList[16], 1), // 23
	    mc_CreateCardBuild("Turret",           "turret",      1, POST_TYRANNY_WORLD,   4, mc_BuildList[2]    ), // 24
	    mc_CreateCardBuild("German Bunker",    "german",      1, POST_TYRANNY_WORLD,   6, mc_BuildList[3]    ), // 25
	    mc_CreateCardTroop("Sniper",           "sniper",      1, POST_TYRANNY_WORLD,   4, mc_TroopList[17], 1), // 26
	    
	    mc_CreateCardTroop("Gaucho",           "gauch",       1, GAUCHO_HISTORY,       5, mc_TroopList[18], 1), // 27
	    mc_CreateCardTroop("Warrior",          "warrior",     1, GAUCHO_HISTORY,       6, mc_TroopList[19], 1), // 28
	    
	    mc_CreateCardTroop("Giant Theropod",   "theropod",    1, DINOSAURS_EPOCH,      8, mc_TroopList[20], 1), // 29
	    mc_CreateCardSpell("Vulcano Eruption", "eruption",    1, DINOSAURS_EPOCH,      5, mc_SpellList[4]    ), // 30
	    
	    mc_CreateCardTroop("Hallucination",    "dababy",      1, ENTRY_ON_HOUSE,       5, mc_TroopList[21], 1), // 31
	};
